...
import net.mcreator.test.procedures.KeyBindGOnKeyPressedProcedure;
...
public class KeyBindGKeyBinding extends TestModElements.ModElement {
	@OnlyIn(Dist.CLIENT)
	private KeyBinding keys;
	public KeyBindGKeyBinding(TestModElements instance) {
		super(instance, 16);
	...
	public void initElements() {
		keys = new KeyBinding("key.test.key_bind_g", GLFW.GLFW_KEY_G, "key.categories.misc");
		...
		if (Minecraft.getInstance().currentScreen == null) {
			if (event.getKey() == keys.getKey().getKeyCode()) {
				if (event.getAction() == GLFW.GLFW_PRESS) {
					TestMod.PACKET_HANDLER.sendToServer(new KeyBindingPressedMessage(0, 0));
		...
		if (type == 0) {
			{
				...
				KeyBindGOnKeyPressedProcedure.executeProcedure($_dependencies);
			}
		}
	}
}
